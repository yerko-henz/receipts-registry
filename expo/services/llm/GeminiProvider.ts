        ...
        responseSchema: {
          ...
          properties: {
            ...
            total: {
              type: Type.STRING,
              description:
                "Exact string as on receipt - the final total/amount due",
            },
            category: {
              type: Type.STRING,
              description:
                "Must be exactly one of: Food, Transport, Utilities, Entertainment, Shopping, Health, Other",
            },
            taxRate: {
              type: Type.NUMBER,
      ...
      const parsedData: ReceiptData = {
        ...
        items: textData.items.map((item) => {
          ...
          let unitPrice = item.unitPrice
            ? parseMonetaryValue(item.unitPrice, userRegion)
            : 0;
        ...
        taxAmount: textData.taxAmount
          ? parseMonetaryValue(textData.taxAmount, userRegion)
          : undefined,
        discount: textData.discount
          ? parseMonetaryValue(textData.discount, userRegion)
          : undefined,

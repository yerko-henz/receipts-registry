...
test.describe('Receipts Page', () => {
  ...
  test('pagination controls should be visible only when needed', async ({ page }) => {
    ...
    await mockReceiptsResponse(page, [{ id: '1', merchant_name: 'Store 1', total_amount: 10, currency: 'USD', created_at: new Date().toISOString() }], 1);
  ...
  test.skip('should verify real data row count and modal details match DB', async ({ page }) => {
    if (process.env.USE_REAL_DATA !== 'true') {
    ...
    const responsePromise = page.waitForResponse((response) => response.url().includes('/rest/v1/receipts') && response.request().method() === 'GET' && response.url().includes('select='));
    ...
    const amountDisplay = modal
      .locator('span, div')
      .filter({ hasText: new RegExp(rawAmount.replace(/\./g, '[,.]')) })
      .first();

...
export function MFASetup({ onStatusChange }: MFASetupProps) {
    const t = useTranslations('userSettings.mfa');
    ...
    const startEnrollment = async () => {
        ...
        try {
            ...
            setError(err instanceof Error ? err.message : t('errors.enroll'));
            setStep('name');
        } finally {
    ...
    const unenrollFactor = async (factorId: string) => {
        ...
        try {
            ...
            setError(err instanceof Error ? err.message : t('errors.unenroll'));
        } finally {
            setActionInProgress(false);
        }
    };
            ...
            <CardContent className="space-y-4">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-500">
                                            {t('list.addedOn', { date: new Date(factor.created_at).toLocaleDateString() })}
                                        </p>
                                    </div>
                                </div>
                                <button
                    ...
                    <div className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <input
                                ...
                                placeholder={t('setup.deviceName.placeholder')}
                                autoFocus
                            />
                            <p className="text-sm text-gray-500">
                                {t('setup.deviceName.helper')}
                            </p>
                        </div>
                        ...
                        <div className="flex justify-end space-x-3">
                            ...
                            >
                                {actionInProgress ? t('buttons.processing') : t('buttons.continue')}
                            </button>
                        </div>
                    </div>
                )}
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                            <label htmlFor="verify-code" className="block text-sm font-medium text-gray-700">
                                {t('setup.verificationInfo.label')}
                            </label>
                            <input
                                id="verify-code"
                                type="text"
                                ...
                                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm"
                                placeholder={t('setup.verificationInfo.placeholder')}
                            />
                        </div>
                        ...
                        <div className="flex justify-end space-x-3">
                            ...
                            <button
                                ...
                                className="flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 disabled:opacity-50"
                            >
                                {actionInProgress ? t('buttons.verifying') : t('buttons.verify')}
                            </button>
                        </div>
                    </div>
                )}
                ...
                {step === 'list' && (
                    <div className="space-y-4">
                        ...
                        >
                            {actionInProgress ? t('buttons.processing') : t('buttons.add')}
                        </button>
                    </div>
                )}
            </CardContent>
